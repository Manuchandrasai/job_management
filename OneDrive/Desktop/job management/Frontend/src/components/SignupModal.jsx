...
const SignupModal = ({ onClose, onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("API signup failed, creating demo user:", error);
      
      // Fallback to demo user creation if API fails
      const demoUser = {
        id: Date.now(),
        name: formData.name,
        email: formData.email
      ...
      <div className="bg-white rounded-lg max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-start">
            <input type="checkbox" required className="rounded border-gray-300 text-purple-600 focus:ring-purple-500 mt-1" />
