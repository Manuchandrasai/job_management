...
function App() {
  ...
  const [salaryRange, setSalaryRange] = useState([3, 80]);
...
const filteredJobs = jobs.filter(job => {
  const salaryValueRaw = job.salary ?? job.salaryMax ?? job.salaryMin;
  const salaryValue = salaryValueRaw ? salaryValueRaw / 100000 : 0; // converting to LPA

  const matchesSalary = salaryValue >= min && salaryValue <= max;
  ...
  return matchesSalary && matchesSearch && matchesLocation && matchesJobType;
});
